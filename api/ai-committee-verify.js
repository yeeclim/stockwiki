...
async function askOllama(question) {
  ...
  if (useLocalFirst) {
    try {
      ...
      console.log('⚠️ Ollama 로컬 서버 연결 실패, 클라우드 서버 시도...');
    }
  }
  
  try {
    // 클라우드 Ollama 사용 (환경변수에서 API 키와 URL 가져오기)
    const apiKey = process.env.OLLAMA_API_KEY;
    const ollamaUrl = process.env.OLLAMA_URL;
    
    // 클라우드 Ollama를 사용하는 경우에만 API 키와 URL 필요
    if (!ollamaUrl) {
      console.log('⚠️ OLLAMA_URL이 설정되지 않아 클라우드 Ollama를 사용할 수 없습니다.');
      throw new Error('OLLAMA_URL이 설정되지 않았습니다.');
    }
    
    // API 키가 있으면 사용, 없으면 인증 없이 시도
    const headers = {
      'Content-Type': 'application/json',
    };
    if (apiKey) {
    ...
    console.error('Ollama 클라우드 오류:', error);
    // 클라우드 실패 시 로컬 서버 시도 (로컬 우선이 아닌 경우에만)
    if (!useLocalFirst) {
      try {
        const localResponse = await fetch('http://localhost:11434/api/generate', {
          method: 'POST',
        ...
        if (localResponse.ok) {
          const localData = await localResponse.json();
          console.log('✅ Ollama 로컬 서버 응답 성공 (폴백)');
          return localData.response;
        }
      } catch (localError) {
        console.error('Ollama 로컬 서버 오류:', localError);
      }
    }
