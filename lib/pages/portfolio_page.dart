...
class _PortfolioPageState extends State<PortfolioPage> {
  ...
  Future<void> _fetchPrices() async {
    ...
    final krCodes = _holdings.where((h) => h.market == 'kr').map((h) => h.stockCode).toList();
    final usTickers = _holdings.where((h) => h.market == 'us').map((h) => h.stockCode).toList();
  ...
  Future<void> _fetchKrPrices(List<String> codes) async {
    for (final code in codes) {
      try {
        final url = 'https://fchart.stock.naver.com/sise.nhn?symbol=$code&timeframe=day&count=2&requestType=0';
        final res = await http.get(Uri.parse(url), headers: {
          'User-Agent': 'Mozilla/5.0',
          'Referer': 'https://finance.naver.com/',
        }).timeout(const Duration(seconds: 8));
        if (res.statusCode == 200) {
          final matches = RegExp(r'data="[^|"]+\|[^|"]+\|[^|"]+\|[^|"]+\|([^|"]+)\|').allMatches(res.body).toList();
    ...
    return Scaffold(
      ...
      body: _isLoading
          ...
          : _holdings.isEmpty
              ...
              : Column(
                  children: [
                    _buildSummary(theme, totalBuy, totalCurrent, totalPnl, totalPnlPct),
                    Expanded(
                      child: RefreshIndicator(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (ctx, i) => _buildCard(theme, _holdings[i]),
                        ),
                      ),
                    ),
                  ],
  ...
  Widget _buildCard(ThemeData theme, PortfolioHolding h) {
    ...
    final pnlPct = hasCur && h.buyPrice > 0 ? (curPrice - h.buyPrice) / h.buyPrice * 100 : 0.0;
    ...
    return Card(
      ...
      child: InkWell(
        onTap: () => showChart(
          context,
          tvSymbol: h.market == 'kr' ? krxSymbol(h.stockCode) : usSymbol(h.stockCode),
              ...
              Row(
                children: [
                  ...
                  _infoCell(theme, '현재가',
                      ...
                      valueColor: hasCur ? theme.colorScheme.onSurface : theme.colorScheme.onSurfaceVariant),
                  const SizedBox(width: 12),
                  _infoCell(theme, '평가손익',
                      hasCur
                          ? '${isProfit ? '+' : ''}${pnlPct.toStringAsFixed(2)}%'
                          : '-',
                      valueColor: hasCur ? pnlColor : theme.colorScheme.onSurfaceVariant),
                ],
              ),
              if (hasCur) ...[
                const SizedBox(height: 8),
  ...
  Widget _infoCell(ThemeData theme, String label, String value, {Color? valueColor}) {
    return Expanded(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
