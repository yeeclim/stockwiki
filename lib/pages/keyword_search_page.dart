...
class _KeywordSearchPageState extends State<KeywordSearchPage> {
    ...
    try {
      ...
      if (_isKoreaSearch) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('한국 주식 검색 기능은 현재 테마별 추천 시스템으로 대체되었습니다.')),
        );
      } else {
        // 미국 주식 검색
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
            _isKoreaSearch ? 'StockWiki – 한국 주식 검색' : 'StockWiki – 미국 주식 검색'),
        actions: [
          IconButton(
        ...
        child: Column(
          children: [
            ...
            if (_isLoading)
              ...
              Text(_isKoreaSearch ? '한국 주식 검색 결과가 없습니다.' : '미국 주식 검색 결과가 없습니다.',
                  style: const TextStyle(color: Colors.white))
            else
              Expanded(
